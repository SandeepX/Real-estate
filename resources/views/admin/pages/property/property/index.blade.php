...
@section('content')
    ...
    <div class="dt-content">
        ...
        <div class="dt-page__header mt-3">
            <h1 class="dt-page__title">All Property </h1>
        </div>
        <!-- /page header -->
                            ...
                            <table id="data-table" class="table table-striped table-bordered table-hover">
                                ...
                                @forelse($properties as $property)
                                    <tr class="gradeX">
                                        ...
                                        <td>{{$property->property_category_id && $property->category ? $property->category->name : 'Undefined'}}</td>
                                        <td>{{$property->property_subcategory_id && $property->subCategory ?$property->subCategory->name : 'Undefined'}}</td>
                                        <td>{{$property->address? $property->address->address : 'Undefined'}}</td>
                                        <td>
                                            @if($property->verify_status == 'verified')
                                                <a href="{{route('admin.request.verification.single',$property->id)}}"  type="button" class="btn btn-sm btn-success pull-left mr-5">
                                                  Activated
                                                </a>
                                            @else
                                                <a href="{{route('admin.request.verification.single',$property->id)}}"  type="button" class="btn btn-sm btn-danger pull-left mr-5">
                                                    Deactivated
                                                </a>
                                            @endif
                                        </td>
